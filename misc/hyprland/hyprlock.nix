...
in
{
  ...
  xdg.configFile."hypr/hyprlock.conf".source = pkgs.writeText "hyprlock-config" ''
    ...
    # Define accent color using Catppuccin Mocha mauve
    # Hyprlock doesn't support variables like $accent, so we hardcode it
    # to the desired Catppuccin color.
    # $accent = $mauve (original concept)
    # $accentAlpha = $mauveAlpha (original concept)
    ...
    image {
        ...
        #path = ~/.face
        size = 100
        border_color = 0x${catppuccinMocha.mauve} # Replaced $accent (which was $mauve) with hex code
    ...
    input-field {
        ...
        dots_spacing = 0.2
        dots_center = true
        outer_color = 0x${catppuccinMocha.mauve} # Replaced $accent (which was $mauve) with hex code
        ...
        placeholder_text = <span foreground="0x${catppuccinMocha.text}"><i >󰌾 Logged in as </i><span foreground="0x${catppuccinMocha.mauve}">$USER</span></span>
        hide_input = false
        check_color = 0x${catppuccinMocha.mauve} # Replaced $accent (which was $mauve) with hex code
